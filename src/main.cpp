...
ros::ServiceClient motorStopClient;
...
void imuCallback(const sensor_msgs::Imu::ConstPtr& msg) {
    ...
    if (z >= 60.0 && z <= 70.0 && !reductionOccurred) {
        std_srvs::Trigger motorStopSrv;
        if (motorStopClient.call(motorStopSrv)) {
...
int main(int argc, char** argv) {
    ...
    motorStopClient = nh.serviceClient<std_srvs::Trigger>("/crawler_control_node/motor_stop");
